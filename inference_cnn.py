...
def prepare_landmarks_for_cnn(landmarks):
    """Prepare landmarks in the format expected by the CNN model."""
    # Initialize array for 33 landmarks with 2 coordinates each
    landmark_array = np.zeros((33, 2))
    
    # Fill the array with normalized coordinates relative to nose
    nose_x = landmarks[0].x
    nose_y = landmarks[0].y
    
    for i, landmark in enumerate(landmarks):
        landmark_array[i, 0] = landmark.x - nose_x  # x coordinate
        landmark_array[i, 1] = landmark.y - nose_y  # y coordinate
    
    # Reshape to match model input shape [1, 33, 2]
    return landmark_array.reshape(1, 33, 2)

# Initialize display window
logger.info("Initializing ExerciseAI Professional (CNN Version)...")
WINDOW_WIDTH = 1920
WINDOW_HEIGHT = 1080
...
logger.info("Loading CNN model and labels...")
data_dir = "data/model"
model = load_model(os.path.join(data_dir, "model_cnn.h5"))
label = np.load(os.path.join(data_dir, "labels_cnn.npy"))
logger.info(f"Model loaded successfully. Available exercises: {len(label)}")
logger.info(f"Available exercise classes: {', '.join(label)}")
...
# Output video setup
output_path = os.path.join(data_dir, "sample_video", "output_cnn.mp4")
...
while cap.isOpened():
    ...
    if res.pose_landmarks:
        ...
        if inFrame(res.pose_landmarks.landmark):
            # Prepare landmarks for CNN model
            landmarks_cnn = prepare_landmarks_for_cnn(res.pose_landmarks.landmark)
            
            # Get predictions from CNN model
            p = model.predict(landmarks_cnn, verbose=0)
            ...
            if confidence > 0.5:
                ...
                info_box_start_y = WINDOW_HEIGHT - 250
    ...
    cv2.imshow("ExerciseAI Professional (CNN)", main_window)
